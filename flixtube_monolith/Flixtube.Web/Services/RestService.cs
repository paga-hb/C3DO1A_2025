...
public class RestService : IRestService
{
    ...
    public RestService(ILogger<RestService> logger, IConfiguration config, HttpClient httpClient)
    {
        ...
        string GATEWAY_SCHEME = _config.GetValue<string>("Flixtube:FLIXTUBE_GATEWAY_SCHEME")!;
        string GATEWAY_HOST = _config.GetValue<string>("Flixtube:FLIXTUBE_GATEWAY_HOST")!;
        int GATEWAY_PORT = _config.GetValue<int>("Flixtube:FLIXTUBE_GATEWAY_PORT")!;
        PublicGatewayBaseUrl = $"{GATEWAY_SCHEME}://{GATEWAY_HOST}:{GATEWAY_PORT}";
    ...
    public async Task<bool> UploadVideoAsync(IBrowserFile file)
    {
        ...
        var message = JsonSerializer.Deserialize<VideoUploaded>(stringContent, _serializerOptions);
        if (message == null) return false;

        // Add Metadata
        Video video = new Video() { Id = message.VideoId, Name = file.Name };
        response = await _httpClient.PostAsJsonAsync<Video>($"/api/metadata", video);

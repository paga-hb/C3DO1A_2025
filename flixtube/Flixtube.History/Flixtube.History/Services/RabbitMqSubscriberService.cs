...
using Flixtube.History.Messages;
using Flixtube.History.Repositories;
using RabbitMQ.Client;
using RabbitMQ.Client.Events;

namespace Flixtube.History.Services;
...
public class RabbitMqSubscriberService : BackgroundService
{
    ...
    private string _queueName = "viewed_queue";
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.Received += async (model, ea) =>
        {
            ...
            var parsedMsg = JsonSerializer.Deserialize<VideoViewedMessage>(message); // Parse the JSON message.
            _logger.LogInformation($"VideoViewedMessage Id: {parsedMsg!.VideoId} ViewedAt: {parsedMsg!.ViewedAt}");

            // Store viewing details about the newly viewed video in the History database
            var history = new ViewHistory { Id = default, VideoId = parsedMsg!.VideoId, ViewedAt = parsedMsg.ViewedAt };
            // _logger.LogInformation($"History Id: {history.Id} VideoId: {history.VideoId} ViewedAt: {history.ViewedAt}");

            using var scope = _serviceScopeFactory.CreateScope();
            var _unitOfWork = scope.ServiceProvider.GetRequiredService<IUnitOfWork>();
            _unitOfWork.ViewHistorys.Add(history);

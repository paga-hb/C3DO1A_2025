...
public class MetadataControllerTests : IClassFixture<PlaywrightFixture>
{
    private readonly IAPIRequestContext _apiContext;
    private readonly JsonSerializerOptions _serializerOptions;
    private readonly ITestOutputHelper _output;

    public MetadataControllerTests(PlaywrightFixture fixture, ITestOutputHelper output)
    {
    ...
    public async Task HttpGet_Should_Return_All_Metadata()
    {
        ...
        await _apiContext.PostAsync("/video", new APIRequestContextOptions { DataObject = expected1 });
        await _apiContext.PostAsync("/video", new APIRequestContextOptions { DataObject = expected2 });

        // Act
        var response = await _apiContext.GetAsync("/videos");
    ...
    public async Task HttpGet_ForSpecificVideo_Should_Return_Video_Metadata()
    {
        ...
        await _apiContext.PostAsync("/video", new APIRequestContextOptions { DataObject = expected });

        // Act
        var response = await _apiContext.GetAsync($"/video/{expected.Id}");
    ...
    public async Task HttpPost_Should_Return_Newly_Added_Metadata()
    {
        ...
        // Act
        var response = await _apiContext.PostAsync("/video", new APIRequestContextOptions { DataObject = expected });
    ...
    public async Task HttpDelete_Should_Return_Newly_Deleted_Metadata()
    {
        ...
        // Act
        var response = await _apiContext.DeleteAsync($"/video/{expected.Id}");
